    ...
    private void initAdapter()
    {
        lvAdapter = new CommonAdapter<NoticeListResponse.NoticeListBean>(mContext, datas, R.layout.item_zhengwu)
        {
            ...
            public void convert(ViewHolder helper, final NoticeListResponse.NoticeListBean item)
            {
                ...
                helper.getView(R.id.llZw).setOnClickListener(new View.OnClickListener()
                {
                    @Override
                    public void onClick(View v)
                    {
                        if (GeneralUtils.isNotNullOrZeroLenght(item.getLink())&&GeneralUtils.isNotNullOrZeroLenght(item.getTitle())){
                            Intent intent = new Intent(mContext, CommonWebViewActivity.class);
                            intent.putExtra(IntentCode.COMMON_WEB_VIEW_TITLE, item.getTitle());
                            intent.putExtra(IntentCode.COMMON_WEB_VIEW_URL, item.getLink());
